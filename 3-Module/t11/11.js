...
// add your code here
for (let p of picArray) {
  document.getElementById('pictures').innerHTML +=
    "<article class='card' onclick='openModal(" + picArray.indexOf(p) + ")'><h2>" + p.title + "</h2> <figure><img src='" + p.image.medium + "' alt='" +
    p.title + "'> <figcaption>" + p.caption + "</figcaption></figure><p>" + p.description + '</p></article>';
}
function openModal(index) {
  const modal = document.querySelector('dialog');
  const modalImage = modal.querySelector('img');

  // Set the large image and alt attribute
  modalImage.src = picArray[index].image.large;
  modalImage.alt = picArray[index].title;
